...
# view first five data frame rows
head(dataset,5)
```
...
```{r echo=TRUE}
mean_steps <- mean(stepsbyday$steps, na.rm=TRUE)
mean_steps
...
2. The missing values were added by using the mean value of the total steps taken in a day.
    a. Do these values differ from the estimates from the first part of the assignment?
        Yes.  Imputing the missing values offsets the results.  
        
    b. What is the impact of imputing missing data on the estimates of the total daily number of steps?  
        The overall distribution of the mean values is not disturbed; however, the imputed values increase the total number of steps on those days where the missing values are added. As a result the median value is shifted to make it equal to the mean value.
    
3. Create a new dataset as the original with the missing values filled in.
...
mean_imputed_steps <- mean(imputed_totals$steps, na.rm=TRUE)
mean_imputed_steps

median_imputed_steps <- median(imputed_totals$steps, na.rm=TRUE)
...
# view first few dataset rows
head(meandayactivity)    
```
